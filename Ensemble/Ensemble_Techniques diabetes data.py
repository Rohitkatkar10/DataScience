# -*- coding: utf-8 -*-
"""
Created on Wed Apr 13 10:21:59 2022

@author: rohit
"""

# Assignment on Ensemble Technique on Diabeted_Ensemble data.

import pandas as pd 
import numpy as np

data = pd.read_csv(r"D:\360digitmg\lectures\19 Ensemble Techniques\Assignment\Datasets\Diabeted_Ensemble.csv")
...
data.rename(columns={'Class variable':'outcome'}, inplace=True)   
data.columns

# we are mapping the outcome column values in 
out = pd.get_dummies(data["outcome"], drop_first='Yes')
out = np.array(out)
data['outcome'] = out
...
features = data.iloc[:, data.columns != 'outcome']
label = data['outcome']   
...
for i in range(len(base_learners)):
    ...
    base_acc.append(round(accuracy_score(y_test, pred)*100,2))
...
# decision tree bagging classifier is overfitting. 
# see random forest classifiers handson for GridSearchCV
...
# this model is good, since no much difference in training and testing accuracy.
